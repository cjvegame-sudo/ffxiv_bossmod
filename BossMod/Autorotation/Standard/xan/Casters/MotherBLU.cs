...
public sealed class MotherBlue(RotationModuleManager manager, Actor player) : Castxan<AID, TraitID>(manager, player, PotionType.Intelligence)
{
    ...
    private bool IsInBurstWindow()
    {
        return Player.FindStatus((uint)SID.WaxingNocturne) != null;
    }
    //Detect Mimicry 
    ...
    public override void Exec(StrategyValues strategy, Enemy? primaryTarget)
    {
        ...
        if (Mimic == Mimicry.DPS)
        {
            ...
            if (!OnCooldown(AID.WingedReprobation) && StatusStacks(SID.WingedReprobation) == 2)
            {
                PushGCD(AID.WingedReprobation, primaryTarget, GCDPriority.BuffRefresh);
                return;
            }

            if (OnCooldown(AID.Nightbloom) && StatusStacks(SID.Bleeding) < 5)
            {
                PushGCD(AID.Bristle, Player);
                PushGCD(AID.SongOfTorment, Player);
            }
